...
\leftline{\bf Discusion:}

Al ver los resultados en los graficos anteriores, algunas de nuestras hipotesis se vieron derrumbadas. Por ejemplo mirando en general los graficos, claramente se ve como en promedio, el mejor algoritmo, el que tiene menor error relativo es el que calcula 1/e\^{}x. Con respecto a esto estuvimos debatiendo el porque de este resultado, y llegamos a la conclusion de que se debe a que, la forma e\^{}(-x) es mas inestable en los primeros terminos, ya que el grafico del el resultado va oscilando entre los positivos y negativos, hasta que comienza a estabilizarse en el valor a calcular, para que suceda esto deben haber mas terminos, y ademas mas presicion. En cambio la forma 1/e\^{}x es mas estable, ya que primero se calcula e\^{}x, donde todos sus terminos son posivos, por lo que no existe el problema de que la serie oscile hasta comenzar a obtener el resultado, lo que significa que no se necesitan tantos terminos, ni tanta presicion para llegar a el resultado. A pesar de que al final, luego de calcular e\^{}x se hace una operacion adicional, que es la de elevar a la potencia negativa -1, el error propagado no es tan significativo, a pesar de que se hace una operacion mas. Por lo tanto queda claro el por que de le resultado.

Otra observacion con respecto a las hipotesis planteadas al principio del articulo, es que la variante del polinomio de taylor que trabaja con los terminos mas pequeños hacia los mas grandes, no resulto tan efectiva como la planeamos. Observando los graficos del uno al cuatro, primero se observa que en los primeros dos, taylor de menor a mayor tiene un menor desempeño con respecto a su contraparte original, si bien la diferencia es casi nula para el metodo 1/e\^{}x, calculando e\^{}(-x) se observa mejor. Viendo esto, llegamos a la conclusion.
...
pcmap.unizar.es/~pilar/latex.pdf

Introduccion a latex

http://es.wikipedia.org/wiki/Análisis\_numérico 

Analisis numerico, redondeo etc

http://babbage.cs.qc.edu/courses/cs341/IEEE-754references.html

Referencia para IEEE 754

webs.uvigo.es/mat.avanzadas/PracME\_1.pdf 
