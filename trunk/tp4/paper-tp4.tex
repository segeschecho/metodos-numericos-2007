...
%esta bien que solemos poner mas detalles que desarrollo pero aca falta bastante, por ejemplo que discretizamos el tiempo, que tenemos 2 splines por misil, la forma rara de triangular, describir mas las funciones de misil, yo hice algo pero no esta todo, separar metodos y miembros.
...
Una variable de tipo bool indica si el misil fue destruido. Este dato es necesario por la forma en la que se decide cuales serán los misiles a explotar. Se utiliza mayormente a fin de no solapar grupos de misiles candidatos en el espacio ni en el tiempo. Por ejemplo, si tengo un conjunto de cuatro misiles en el tiempo $t$, y decidí destruirlo, otra hipotética formación de tres proyectiles, en la cual se encuentre alguno de los ya destruidos (en el instante $t$ o posterior) será tenida en cuenta con solo dos misiles.

Luego se tiene un arreglo con los coeficientes correspondientes a los polinomios calculados con el método de splines. Estos se utilizan con un par de funciones llamadas posicionX y posicionY, que evaluan las funciones recíprocas para un instnate determinado por el llamador, obteniendo asi las coordenadas con respecto al tiempo sobre el eje de las absisas y el de las ordenadas. Un uso conjunto de estos métodos permite conocer la posición del misil en cualquier intervalo de nuestra discretización del tiempo.
